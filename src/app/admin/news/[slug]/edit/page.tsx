...
import { ArrowLeft, Save, ImageIcon, Check } from 'lucide-react'
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-5 gap-3 max-h-96 overflow-y-auto p-2 bg-gray-50 rounded-lg">
              {artworks.map(artwork => {
                const isSelected = formData.images?.heroArtworkId === artwork.id ||
                  (formData.images?.hero === artwork.images.thumbnail)
                return (
                  <button
                    key={artwork.id}
                    type="button"
                    onClick={() => updateHeroImage(artwork.id)}
            ...
            <p className="text-xs text-gray-400 mt-2">
              선택한 작품의 이미지가 뉴스 기사의 대표 이미지로 표시됩니다.
            </p>
          </div>
        </div>
